...
function IBS_InitSheet(sheet, info) {
	...
	//시트에 공통으로 설정 해야 할 내용을 설정
	sheet.FitColWidth(); 
	/*
	 ibsheet.cfg 파일에 AutoFitColWidth:init를 선언하면 초기화와 동시에 조회해야 하는 화면일 경우 화면이 갱신되는게 보이는데 이를 방지하기 위하여
	 ibsheet.cfg 에서 init를 설정 하지 않고 InitColumns 함수 아래에서 FitColWidth를 설정함
	*/
	
	//sheet.SetExtendLastCol(1);
	//시트에 공통으로 설정 해야 할 내용을 설정 끝
