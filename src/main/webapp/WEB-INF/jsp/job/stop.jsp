				...
				<ul class="nav metismenu" id="side-menu">
					<li class="active"><a href="#"><i class="mnImg02"></i><span class="nav-label">내작업</span><span class="arrow01"></span></a>
						<ul class="nav nav-second-level">
							...
							<li class="active"><a href="${pageContext.request.contextPath}/job/stop">중단</a></li>
					...
					<div class="sectionCon">
						<div class="board_top mainSearch">
							...
							<div class="col-md-8 text-right"  style="padding-left: 0px;">
								<a href="${pageContext.request.contextPath}/job/subregister" class="btn btn-info" title=""><i
									class="ti-plus"></i>&nbsp;&nbsp;새작업</a>
								</div>
							</div>
							<!-- 버튼 끝 -->
						</div>
						...
						<div class="myWork">
							...
							<c:forEach var="stop" items="${stoplist}">
								<c:choose>
									...
									<c:if test="${stop.project_id ne 0}"><a href="/project/main?id=<c:out value="${stop.project_id}" />" title="프로젝트 이동"> </c:if>
									<div class="cate">
										<c:out value="${stop.project.project_nickname eq null ? '기타' : stop.project.project_nickname}" />
										<c:out value="${stop.project.project_nickname eq null ? '' : stop.order_id}" />
										<c:choose>
			                                  <c:when test="${stop.week==1}">
									...
									<div class="myWorkListTit">
										<a href="/job/submodify?id=<c:out value="${stop.id}"/>">
										<h4 title="작업명(하위작업 개수)">
											<c:out value="${stop.name}" />(<c:out value="${stop.sub_cnt}" />)<span></span>
											</h4>
											<fmt:formatNumber value="${stop.real_progress*100}" type="number" var="numberType" />
	                      						
											<p><c:out value="${numberType}"/>%</p>
										</a> 
									</div>
									<div style="margin-top: 5px;">
										(<c:out value="${stop.real_start_date!=undefined? stop.real_start_date : stop.start_date }"/>)~(<c:out value="${stop.real_end_date!=undefined? stop.real_end_date : stop.end_date}"/>)
									</div>
								</div>
								<!--myWorkWrap end  -->
							  </c:forEach>	
