				...
				<ul class="nav metismenu" id="side-menu">
					<li class="active"><a href="#"><i class="mnImg02"></i><span class="nav-label">내작업</span><span class="arrow01"></span></a>
						<ul class="nav nav-second-level">
							...
							<li class="active"><a href="${pageContext.request.contextPath}/job/ing">진행중  <c:out value="${inglastCnt}" /></a></li>
						...
						<div class="myWork">
							...
							<c:forEach var="ing" items="${inglist}">
								<c:choose>
	                                  ...
	                                  <%--  <c:when test="${ing.wbs_id!=0 && ing.real_end_date < today}">
									...
									<div class="myWorkListTit">
										<a href="/job/submodify?id=<c:out value="${ing.id}"/>">
											<%-- <h4>
											<c:out value="${ing.name}" />(하위: <c:out value="${ing.sub_cnt}" />개)<span></span>
											</h4> --%>
											<c:choose>
													<c:when test="${ing.real_end_date < today}">
				                                     	<h4 style="color: #FF0000" title="작업명(하위작업 개수)"><c:out value="${ing.name}"/>(<c:out value="${ing.sub_cnt}" />)<span></span></h4>
				                                   </c:when>
				                                   <c:when test="${ing.real_end_date >= today}">
														<h4 title="작업명(하위작업 개수)"><c:out value="${ing.name}"/>(<c:out value="${ing.sub_cnt}" />)<span></span></h4>
												 	</c:when>
			                              	</c:choose>
									...
									<div style="margin-top: 5px;">
										(<c:out value="${ing.real_start_date!=undefined? ing.real_start_date : ing.start_date }"/>)~(<c:out value="${ing.real_end_date!=undefined? ing.real_end_date : ing.end_date}"/>)
									</div>
								</div>
								<!--myWorkWrap end  -->
							  </c:forEach>	
